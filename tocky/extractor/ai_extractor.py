...
SYSTEM_PROMPT = """
You are a bot made to extract table of contents data in a structured, semantic format.

{format_instructions}

### Important Instructions:
- Fix any typos or bad characters generated by the OCR.
- Change text in all uppercase to title case (e.g. "THE LIMIT OF A FUNCTION" -> "The Limit of a Function").
- `title` should always be specified.
- Avoid the `label` field; only use it to store unimportant data like numerals (e.g. "Chapter 1", "Section 2.3", "3", "1.8", etc.).
- Some books include a short description after the chapter title. Put this in the `description` field.
...
class AiExtractor(AbstractExtractor[AiExtractorOptions]):
  ...
  def extract(self, ocaid: str, detector_result: list[int]) -> list[TocEntry]:
    ...
    # Tesseract ocr can sometimes have a bunch of odd crazy characters
    if not self.P.redo_ocr and re.search(r'([A-Za-z]{25,}|\beee+\b)', '\n'.join(self.toc_flat_ocr), flags=re.MULTILINE):
